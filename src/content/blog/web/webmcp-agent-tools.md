...
WebMCP는 Agent가 화면의 DOM과 좌표를 추측해 조작하는 대신, 웹페이지가 의도·입력 schema·실행 경로를 갖춘 작은 도구를 직접 제공하자는 제안입니다. 이 구조는 자동화를 더 안정적으로 만들 수 있지만 인증·인가·쓰기 작업의 확인을 대신하지는 않으므로, 읽기 전용의 좁은 작업 계약과 eval부터 점진적으로 도입해야 합니다.
