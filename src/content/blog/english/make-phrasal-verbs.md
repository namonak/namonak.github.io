---
title: "make 구동사 정리: make up, make out, make of 뜻과 예문"
...
이 글에서는 `make` 계열 구동사를 묶어서 정리합니다. `make up`, `make out`, `make of`처럼 의미가 많이 갈라지는 표현을 중심으로 살펴봅니다.
...
`make up`은 의미가 매우 많은 구동사입니다. 기본적으로는 **없는 것을 만들어 내거나, 부족한 것을 채우거나, 여러 요소가 모여 전체를 이루는 이미지**가 있습니다.
...
부족하거나 놓친 것을 채우는 의미입니다. 이때는 상황에 따라 `make up` 또는 `make up for`를 씁니다.
...
`make up`은 화장을 하다는 의미로도 쓰입니다. 다만 명사 `makeup`은 “화장품” 또는 “화장”이라는 뜻입니다.
...
`make money off of ~`는 엄밀히 말하면 전형적인 구동사라기보다는 **관용적인 전치사 표현**에 가깝습니다. 의미는 **~을 통해 돈을 벌다**, **~로부터 수익을 얻다**입니다.

`off of`는 특히 미국 영어에서 자주 쓰이는 다소 구어적인 표현입니다. 더 일반적이고 깔끔한 표현으로는 `make money from ~`을 쓸 수 있습니다.
...
`make away`는 단독으로는 현대 영어에서 자주 쓰이지 않습니다. 보통은 아래와 같은 형태로 더 많이 쓰입니다.
...
`make off`는 “급히 떠나다”, “도망치다”라는 의미로도 쓰입니다.
...
`make something of ~`는 어떤 사람, 일, 기회, 사업 등을 **가치 있는 것으로 만들어 내는 것**입니다.
...
`make something of oneself`는 자기 자신을 발전시켜 성공하거나 의미 있는 사람이 되다는 뜻입니다.
...
이 표현은 단순히 돈을 많이 번다는 뜻만은 아닙니다. **가능성이나 자원을 잘 활용해서 의미 있는 결과를 만든다**는 느낌이 강합니다.
...
`make of`는 어떤 사람, 사건, 말, 상황을 보고 **그것을 어떻게 이해하거나 해석하는지**를 말할 때 씁니다.
...
`make it up to someone`은 자신이 저지른 잘못, 실수, 피해를 **상대에게 보상하거나 만회하는 것**입니다.
...
`make it up`만 쓰면 문맥에 따라 “그것을 지어내다”라는 뜻으로 들릴 수 있습니다. “누군가에게 보상하다”라는 의미를 분명히 하려면 보통 `to someone`을 붙입니다.
...
`make out`의 기본 이미지는 **희미하거나 불분명한 것을 알아내는 것**입니다. 여기서 “간신히 알아듣다/알아보다”, “~인 척하다/주장하다”, “문서를 작성하다” 등의 의미로 확장됩니다.
...
`make out that + 문장`은 실제와 다르게 어떤 것이 사실인 것처럼 말하거나 주장한다는 뜻입니다.
...
이 의미의 `make out`은 상대를 속이거나 과장하는 느낌이 들어갈 수 있습니다. 즉, 단순히 “말했다”보다 **그렇게 보이게 만들었다 / 그런 척했다**에 가깝습니다.
...
`make out`은 어떤 상황에서 어떻게 지내거나 해내고 있는지를 말할 때도 씁니다.
...
`make out`은 연인이나 커플이 진하게 키스하거나 애정 표현을 한다는 뜻으로도 쓰입니다. 다소 구어적이고 사적인 표현입니다.
